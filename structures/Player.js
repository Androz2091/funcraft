...
module.exports = class Player {
    constructor($, period) {
        ...
        if (
            ['Admin', 'Joueur', 'SuperModo', 'Modo', 'Helper', 'Youtuber', 'Builder', 'Graphiste'].includes(
                playerName.children[1].children[0].data.trim().split(' ')[0]
            )
        ) {
            ...
            try {
                ...
                if (e instanceof TypeError) {
                    this.grade = '';
                    for (let i = 1; i < playerName.children.length - 2; i++)
                        this.grade += playerName.children[i].children[0].data;
                    error = true;
                }
            }
        ...
        this.gloryCount = parseInt(
            $('.info-stats').get(0).children[1].children[2].data.trim().split(' Gloires')[0].split(' ').join('')
        );
